...
function getLocalizedUrl(projectSlug) {
  ...
  if (language === 'fr') {
    return `/fr/fondation-pour-les-arbres-nos-projets/${projectSlug}`;
  } else {
...
function createInfoBoxContent(imageId, imageName, partner, country, dateRange, title, category, projectSlug) {
  ...
  return `<div class='myboxmap'><img src='https://content.da.live/audemars-piguet/arbres-fondationsaudemarspiguet/assets/images/map/projects-listing/${imageId}/${imageName}.jpg' width='220' height='144'><p class="carte_box_partenaires">${partner}</p><p class="carte_box_pays">${country}</p><p class="carte_box_date">${dateRange}</p><p class="carte_box_titre">${title}</p><p class="carte_box_categorie">${category}</p><p class="carte_box_link"><a href=${localizedUrl}>${seeMoreText}</a></p></div>`;
}
